...
#
# Замер окна модели: запрос на 442 137 токенов промпта прошёл без ошибки, то есть
# ограничение провайдера как минимум там. 400 000 символов русского текста — это
# примерно 130-200 тыс. токенов, то есть меньше половины замеренного потолка.
# Держим запас намеренно: провайдер лимит не публикует, упереться в него посреди
# диалога хуже, чем подрезать самую старую часть истории.
#
# Совсем снимать предел нельзя: входные токены оплачиваются на каждой итерации
# цикла, а их за один вопрос бывает 4-6.
...
КАК НАЗЫВАТЬ ПРИЗНАКИ
Пользователь — специалист по ликвидности, а не разработчик.

- Пиши ТОЛЬКО человеческое название признака (поле label). Технические имена
  колонок (m3x_cover, m2_base_cover_mad) в ответ не выноси вообще: они не несут
  для читателя смысла. Имена колонок нужны тебе для запросов, а не ему для чтения.
- Служебные поля инструментов — higher_means, status, source, label_status —
  это данные для ТЕБЯ. Никогда не печатай их в ответе ни в скобках, ни иначе.
  Строчки вида «(status: derived)» или «(higher_means: stress)» недопустимы.
- Направление знака бери из higher_means и выражай обычными словами: «рост
  означает движение в сторону стресса». Не угадывай направление по имени: у
  «Стресса покрытия ОФЗ» знак обратный, положительное значение — это недоспрос.
- Если формулировка названия не подтверждена (status needs_review или missing),
  скажи это человеческой фразой — «название признака ещё уточняется» — и только
  когда строишь на нём содержательный вывод.
- Служебные флаги доступности данных не выдавай за рыночный сигнал.
